...
public class CucumberReporter implements Formatter {
	...
	public void setEventPublisher(EventPublisher publisher) {
		...
		publisher.registerHandlerFor(TestCaseFinished.class, new EventHandler<TestCaseFinished>() {
	...
	private void handleTestCaseStarted(TestCaseStarted event) {
		...
		String name = getTestCaseName(event.testCase);
		Map<String, String> attributes = new HashMap<>();
		attributes.put(bddTestrunUUIDPropName, uuid);
		if (currentFeatureMap != null && currentFeatureMap.get("name") != null) {
			attributes.put("BDD_FEATURE_NAME", currentFeatureMap.get("name").toString());
		}
		...
		if (currentTestCaseMap.get("name") != null) {
			attributes.put("BDD_TESTCASE_NAME", currentTestCaseMap.get("name").toString());
		}
		if (currentTestCaseMap.get("name") != null) {
			attributes.put("BDD_TESTCASE_DESCRIPTION", currentTestCaseMap.get("description").toString());
		}
		if (currentTestCaseMap.get("name") != null) {
	...
	private void handleTestStepStarted(TestStepStarted event) {
		if (event.testStep instanceof PickleStepTestStep) {
			...
			if (currentStepOrHookMap.get("line") != null) {
				attributes.put("BDD_STEP_NAME", currentStepOrHookMap.get("name").toString());
			}
			if (currentStepOrHookMap.get("line") != null) {
